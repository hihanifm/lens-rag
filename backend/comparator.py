...
# ── LLM Judge ─────────────────────────────────────────────────────────────
# Empty `llm_judge_prompt` → DEFAULT_LLM_JUDGE_PROMPT (full built-in system message).
# Non-empty → that text is the full system message (no server prefix/suffix; _parse_llm_judge_batch).

# Shown in GET /compare/llm-judge-defaults for UI — built-in default is monolithic (no separate appended suffix).
LLM_JUDGE_PROMPT_SUFFIX = ""

DEFAULT_LLM_JUDGE_PROMPT = """You compare test specifications from two sources, such as different clients, carriers, or baselines.
...
def effective_llm_judge_system_prompt(domain_overlay: str | None) -> str:
    ...
    Empty or whitespace → built-in DEFAULT_LLM_JUDGE_PROMPT.
